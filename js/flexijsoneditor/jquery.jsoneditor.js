...
(function( $ ) {
    ...
    $.fn.jsonEditor = function(json, options) {
        ...
        return this.each(function() {
            JSONEditor($(this), json, onchange, onpropertyclick, options.propertyElement, options.valueElement, comments);
        });
        
    };
    
    function JSONEditor(target, json, onchange, onpropertyclick, propertyElement, valueElement, comments) {
        var opt = {
            target: target,
            onchange: onchange,
    ...
    function addCheckbox(item, input, value, changed){
        let checkbox = $('<input>', { 'class': 'checkbox', 'type': 'checkbox' })
        input.change(() => {
            if (checkbox.prop('checked') == (input.prop('value') == 'true')) return
            checkbox.prop('checked', input.prop('value') == 'true')
        })
        checkbox.prop('checked', value)
        checkbox.change(() => {
            input.prop('value', checkbox.prop('checked') ? 'true' : 'false')
            changed.apply(input)
        })
    ...
    function construct(opt, json, root, path) {
        ...
        for (var key in json) {
            ...
            if (isBoolean(json[key])) {
                addCheckbox(item, value, json[key], valueChanged(opt))
            }

            let valuePath = (path ? path + '.' : '') + key
            if (opt.comments[valuePath]) {
                let comment = $('<div>', { 'class': 'comment' });
                comment.text('// ' + opt.comments[valuePath]);
                item.append(comment);
            }
            root.append(item);
